...
function fieldcheck(id){  //check if the field represented by the unique identifier id is valid.
   ...
   //test for email
    ...
    if (id == "bookingref") {                       //Allow valid booking reference
        pattern = /[a-z A-Z]\-\d\d\d\d/;     // test the value against an appropriate regular expression 
        failmessage = "Only valid booking reference are permitted here";
    }
...
function validate() {
    ...
    valid = valid && fieldcheck("bookingref");
    return valid;
}
