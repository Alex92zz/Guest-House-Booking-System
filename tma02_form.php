...
?>
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" name="tt284guestform<?php echo $row["id"]; ?>" onsubmit="return validate()">
        ...
        <p><label for="bookingref">Booking Reference: </label>
            <input type="text" name="bookingref" id="bookingref" maxlength="8" minlength="8" value="<?php if ($valid){echo htmlspecialchars($row["bookingref"]); }else{echo htmlspecialchars($webdata["bookingref"]);}?>" onchange="fieldcheck(this.id)">
            <span id="fbbookingref"> </span><?php if (!$valid){echo  $formerror["bookingref"]  ;} ?>
        </p>
